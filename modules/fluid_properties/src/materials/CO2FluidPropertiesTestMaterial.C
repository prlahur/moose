...
InputParameters validParams<CO2FluidPropertiesTestMaterial>()
{
  ...
  params.addParam<bool>("vapour", false, "Flag to calculate vapour pressure");
...
CO2FluidPropertiesTestMaterial::CO2FluidPropertiesTestMaterial(const InputParameters & parameters) :
    ...
    _pvap(declareProperty<Real>("vapour_pressure")),
    _rhovap(declareProperty<Real>("saturated_vapour_density")),
...
CO2FluidPropertiesTestMaterial::computeQpProperties()
{
  ...
  _pvap[_qp] = (_vapour ? _fp.vapourPressure(_temperature[_qp]) : 0.0);
  _rhovap[_qp] = (_vapour ? _fp.saturatedVapourDensity(_temperature[_qp]) : 0.0);
  _rhosat[_qp] = (_vapour ? _fp.saturatedLiquidDensity(_temperature[_qp]) : 0.0);
