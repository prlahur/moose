...
BrineFluidPropertiesTestMaterial::computeQpProperties()
{
  ...
  _rho_halite[_qp] = _fp.rhoHalite(_pressure[_qp], _temperature[_qp]);
  _cp_halite[_qp] = _fp.cpHalite(_pressure[_qp], _temperature[_qp]);
  _h_halite[_qp] = _fp.hHalite(_pressure[_qp], _temperature[_qp]);

  // Calculate the water density as it is used to calculate viscosity and
  // thermal conductivity
  Real rhow = _fp.rhoWater(_pressure[_qp], _temperature[_qp]);
