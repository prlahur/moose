...
void
SinglePhaseFluidPropertiesPT::henryConstantIAPWS_dT(Real temperature, Real & Kh, Real & dKh_dT, Real A, Real B, Real C) const
{
  Real pc = 22.064e6;
  Real Tc = 647.096;
  ...
  Real lnkh = A / Tr + B * std::pow(tau, 0.355) / Tr + C * std::pow(Tr, -0.41) * std::exp(tau);
  Real dlnkh_dT = (- A / Tr / Tr - B * std::pow(tau, 0.355) / Tr / Tr - 0.355 * B *
   std::pow(tau, -0.645) / Tr - 0.41 * C * std::pow(Tr, -1.41) * std::exp(tau) -
   C * std::pow(Tr, -0.41) * std::exp(tau)) / Tc;
  ...
  Real p = pc * std::exp(sum / Tr) * std::exp(lnkh);
  Real dp_dT = pc / Tc * (- sum / Tr / Tr - dsum / Tr) * std::exp(sum / Tr);

  // Henry's constant and its derivative wrt temperature
  Kh = pc * std::exp(sum / Tr) * std::exp(lnkh);
