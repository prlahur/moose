...
InputParameters validParams<PorousFlowComponentMass>()
{
  ...
  params.addParam<unsigned int>("component_index", 0, "The index corresponding to the component for this Postprocessor");
  ...
  params.addParam<std::vector<unsigned int> >("phase_index", "The index of the fluid phase that this Postprocessor applies to. Default is all phases");
...
{
  /// Check that the component_index is not greater than the number of components
  if (_component_index >= _dictator_UO.num_components())
    mooseError("The Dictator proclaims that the number of components in this simulation is " << _dictator_UO.num_components() << " whereas you have used the Postprocessor PorousFlowComponentMass with component = " << _component_index << ".  The Dictator does not take such mistakes lightly");
