...
InputParameters validParams<PorousFlowChemistryDispersiveFlux>()
{
  ...
  params.addClassDescription("Dispersive and diffusive flux of the generalised concentration given by primary_species in all phases");
  return params;
}

PorousFlowChemistryDispersiveFlux::PorousFlowChemistryDispersiveFlux(const InputParameters & parameters) :
    Kernel(parameters),
    ...
    _grad_gen_concentration(getMaterialProperty<std::vector<std::vector<RealGradient> > > ("PorousFlow_grad_gen_conc")),
...
Real
PorousFlowChemistryDispersiveFlux::computeQpResidual()
{
  ...
  {
    // Diffusive component
    diffusion = _porosity_qp[_qp] * _tortuosity[_qp][ph] * _diffusion_coeff[_qp][ph][_primary_species];
    ...
    flux += _fluid_density_qp[_qp][ph] * (diffusion * _identity_tensor + dispersion) * _grad_gen_concentration[_qp][ph][_primary_species];
  }
  return _grad_test[_i][_qp] * flux;
}

Real
PorousFlowChemistryDispersiveFlux::computeQpJacobian()
{
  return computeQpJac(_var.number());
}

Real
PorousFlowChemistryDispersiveFlux::computeQpOffDiagJacobian(unsigned int jvar)
{
  return computeQpJac(jvar);
}

Real
PorousFlowChemistryDispersiveFlux::computeQpJac(unsigned int jvar) const
{
  ...
  for (unsigned int ph = 0; ph < _num_phases; ++ph)
  {
    ...
    Real ddiffusion = _phi[_j][_qp] * _dporosity_qp_dvar[_qp][pvar] * _tortuosity[_qp][ph] * _diffusion_coeff[_qp][ph][_primary_species];
    ddiffusion += _phi[_j][_qp] * _porosity_qp[_qp] * _dtortuosity_dvar[_qp][ph][pvar] * _diffusion_coeff[_qp][ph][_primary_species];
    ddiffusion += _phi[_j][_qp] * _porosity_qp[_qp] * _tortuosity[_qp][ph] * _ddiffusion_coeff_dvar[_qp][ph][_primary_species][pvar];
    ...
    dflux += _phi[_j][_qp] * _dfluid_density_qp_dvar[_qp][ph][pvar] * (diffusion * _identity_tensor + dispersion) * _grad_gen_concentration[_qp][ph][_primary_species];
    dflux += _fluid_density_qp[_qp][ph] * (ddiffusion * _identity_tensor + ddispersion) * _grad_gen_concentration[_qp][ph][_primary_species];
    dflux += _fluid_density_qp[_qp][ph] * (diffusion * _identity_tensor + dispersion) * _dgen_concentration_dvar[_qp][ph][_primary_species][pvar] * _grad_phi[_j][_qp];
  }
