...
PorousFlowFluidStateWaterNCG::thermophysicalProperties()
{
  ...
  if (is_gas || is_twophase)
  {
    ...
    Real ncg_density2, dncg_density2_dp, dncg_density2_dT;
    Real vapor_density2, dvapor_density2_dp, dvapor_density2_dT;
    _ncg_fp.rho_dpT(
        Y0 * _gas_porepressure[_qp] + 1.0e-2, Tk, ncg_density2, dncg_density2_dp, dncg_density2_dT);

    _water_fp.rho_dpT(
        (1.0 - X0) * psat, Tk, vapor_density2, dvapor_density2_dp, dvapor_density2_dT);

    Real fd1 = (ncg_density2 - ncg_density) / 1.0e-2;
    Real fd = (ncg_density2 + vapor_density2 - ncg_density - vapor_density) / 1.0e-2;
    ...
    _console << "dncg_density_dp " << dncg_density_dp << ", fd " << fd1 << std::endl;

    _console << "dgas_density_dp " << dgas_density_dp << ", fd " << fd << std::endl;
    _console << "dgas_density_dz " << dgas_density_dz << std::endl;
